...
public class SurfaceTextureHelper {
     ...
     */
    public void setTextureSize(final int textureWidth_, final int textureHeight_) {
        if (textureWidth_ <= 0) {
            throw new IllegalArgumentException("Texture width must be positive, but was " + textureWidth_);
        }
        if (textureHeight_ <= 0) {
            throw new IllegalArgumentException(
                    "Texture height must be positive, but was " + textureHeight_);
        }
        surfaceTexture.setDefaultBufferSize(textureWidth_, textureHeight_);
